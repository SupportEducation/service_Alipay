...
/**
 * Servlet implementation class GetAllFriends
 */
...
public class GetAllFriends extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String aid=new String(request.getParameter("aid").getBytes(
				"ISO-8859-1"), "UTF-8");
		List<follow> foList=new followDao().getOneUserFollow(Integer.parseInt(aid));
		List<friends> friList=new ArrayList<>();

		for(follow fo:foList){
			friends fri=new friends();
			account ac=new accountDao().getOneAccount(fo.getAaid());
			fri.setAaid(fo.getAaid());
			fri.setHeadimg(ac.getHeadimg());
			fri.setNickname(ac.getNickname());
			fri.setFansum(new followDao().getByFollowSum(fo.getAaid()));
			fri.setTeasum(new vorecruitDao().getFlat1NumByaid(fo.getAaid()));
			friList.add(fri);
		}
