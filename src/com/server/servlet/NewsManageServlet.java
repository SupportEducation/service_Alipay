...
public class NewsManageServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int nid =Integer.parseInt(new String(request.getParameter("nid").getBytes(
				"ISO-8859-1"), "UTF-8"));
		boolean flat=new NewsDao().DeleteNews(nid);
	        if (flat==true) {
	      ...
	      //上传完毕后  转发到首页
	        request.getServletContext().getRequestDispatcher("/NewsManage.jsp").forward(
	                request, response);
	        out.flush();
			out.close();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 检测是否为多媒体上传
		     ...
		     //  String uploadPath=s1.getRealPath("/")+ UPLOAD_DIRECTORY;
		                    ...
		                    else
		                    {
		                    	...
		                    	if ("news_content".equals(item.getFieldName()))
		                    		ne.setNews_content(new String(item.getString().getBytes(
		                				"ISO-8859-1"), "UTF-8"));
		                    	if ("nid".equals(item.getFieldName()))
		                    		ne.setNid(Integer.parseInt(new String(item.getString().getBytes(
			                				"ISO-8859-1"), "UTF-8")));
		                    	if ("img".equals(item.getFieldName()))
		                    		ne.setNews_img(new String(item.getString().getBytes(
			                				"ISO-8859-1"), "UTF-8"));
		                    }
		                }
		            }
		        ...
		        if(UPLOAD_WORKNAME!=""&&UPLOAD_WORKNAME!=null)
		        	ne.setNews_img(UPLOAD_WORKNAME);
		        	
		        response.setContentType("text/html;charset=UTF-8");
				PrintWriter out = response.getWriter();
				boolean flat=new NewsDao().updateNews(ne);
			        if (flat==true) {
						request.setAttribute("message",
			                    "修改成功!");
					} else {
			        ...
			        request.setAttribute("nid", ne.getNid());
			      //上传完毕后  转发到首页
			        request.getServletContext().getRequestDispatcher("/NewsEdit.jsp").forward(
			                request, response);
			        out.flush();
					out.close();
