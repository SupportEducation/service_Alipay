...
/**
 * Servlet implementation class Or_updateRecruit
 */
@WebServlet("/OrganizationWeb/Or_updateRecruit")
public class Or_updateRecruit extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
     ...
     //  String uploadPath=s1.getRealPath("/")+ UPLOAD_DIRECTORY;
                    ...
                    else
                    {
                    	...
                    	if ("re_peoplenumber".equals(item.getFieldName()))
                    		re.setRe_peopleenumber(new String(item.getString().getBytes(
	                				"ISO-8859-1"), "UTF-8"));
                    	if ("re_rid".equals(item.getFieldName()))
                    		re.setRid(Integer.parseInt(new String(item.getString().getBytes(
	                				"ISO-8859-1"), "UTF-8")));
      ...
      //将信息存入 数据库
        ...
        re.setOid(Integer.parseInt(request.getSession().getAttribute("oid").toString()));
        
        boolean flat=new recruitDao().updateRecruit(re);
        if (flat==true) {
			request.setAttribute("message",
                    "修改成功!");
			request.setAttribute("rid", re.getRid());
		} else {
			request.setAttribute("message",
                    "修改失败!");
			request.setAttribute("rid", re.getRid());
      ...
      //上传完毕后  转发到首页
        request.getServletContext().getRequestDispatcher("/OrganizationWeb/Or_release_update.jsp").forward(
                request, response);
        out.flush();
		out.close();
	}
